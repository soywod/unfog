...
parseDate now defHours defMins defSecs (ArgDate Rel d mo y h m) =
  Just
...
eval tree arg = case arg of
  ...
  AddWord desc -> tree {_desc = desc'}
    where
      desc' = if null (_desc tree) then desc else _desc tree ++ " " ++ desc
